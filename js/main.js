function App() {
    const conf = {
      nx: 16,
      ny: 80,
      cscale: chroma.scale(['#2175D8', '#DC5DCE', '#CC223D', '#F07414', '#FDEE61']).mode('lch'),
      darken: -1.5,
      angle: -Math.PI/3,
      timeCoef: 0.06 };
    ...
    function initScene() {
      ...
      const vertexShader = `
        ...
        void main() {
          ...
          normal *= 1.0
            + uRnd5.x * (cos((position.y + rnd1.x) * 2.5 * rnd1.y) + 0.5)
            + uRnd5.y * (sin((position.y + rnd2.x) * 2.5 * rnd2.y) + 3.0)
            + uRnd5.z * (cos((position.y + rnd1.z) * 10.0 * rnd2.z) + 4.0);
          pos.xy -= normal * side;
